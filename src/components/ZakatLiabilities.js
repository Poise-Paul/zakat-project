...
const LiabilityManager = () => {
  ...
  // 🔑 Dynamic Totals Calculation using the converter
  const totalLiability = liabilities.reduce(
  ...
  const getCategoryTotal = (category) =>
    liabilities
      .filter((l) => l.liabilityClass === category)
      .reduce((acc, curr) => acc + convertToNGN(curr.amount, curr.currency), 0);
  ...
  // 🔑 Fetch Rates and Liabilities on mount
  useEffect(() => {
    ...
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <div className="flex flex-col items-center my-10 px-4">
        <div className="w-full max-w-5xl bg-white border border-gray-200 shadow-xl rounded-2xl overflow-hidden">
          ...
          <div className="min-h-[250px]">
            {loading ? (
              <div className="p-20 text-center font-bold text-green-700 animate-pulse">
                Loading Liabilities...
              </div>
            ) : liabilities.length === 0 ? (
              <div className="flex justify-center items-center h-[250px] text-gray-400 italic">
              ...
              liabilities.map((item) => (
                ...
                >
                  ...
                  <div className="flex flex-col items-center justify-center">
                    ...
                    {item.currency !== "NGN" && exchangeRates && (
                      <span className="text-[10px] text-gray-400 font-medium mt-0.5">
                        (~₦
                        {Math.round(
                          convertToNGN(item.amount, item.currency),
                        ).toLocaleString()}
                        )
                      </span>
                    )}
          ...
          <div className="bg-gray-50 p-8 border-t-2 border-gray-100 flex justify-end">
            <div className="text-right">
              ...
              <span className="text-4xl font-black text-gray-800">
                NGN {Math.round(totalLiability).toLocaleString()}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-8 w-full max-w-xl shadow-2xl">
          ...
          <div className="flex flex-col gap-5">
            <div className="flex flex-col gap-1">
              ...
              <input
                type="text"
                className="bg-gray-100 p-3 rounded-xl outline-none focus:border-green-500 border-2 border-transparent"
                value={formData.description}
                placeholder="Enter Description"
                onChange={(e) =>
                  setFormData({ ...formData, description: e.target.value })
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <select
                  className="bg-gray-100 p-3 rounded-xl outline-none cursor-pointer focus:border-green-500 border-2 border-transparent"
                  value={formData.currency}
                  onChange={(e) =>
                    setFormData({ ...formData, currency: e.target.value })
                  }
